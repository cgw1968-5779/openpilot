...
class CarInterface(CarInterfaceBase):
  ...
  def _get_params(ret, candidate, fingerprint, car_fw, experimental_long, docs):
    ...
    if candidate in TSS2_CAR or ret.enableGasInterceptor:
      ...
      tune.kiV =  [.33, .30,  .225,  .213,  .19,  .17,  .10, .001] if dp_toyota_enhanced_long_tune else [.35, .23, .20, .17, .1]
      ...
      if candidate in TSS2_CAR:
        ...
        ret.stoppingDecelRate = 0.1 if dp_toyota_enhanced_long_tune else 0.3 # reach stopping target smoothly - seems to take 0.5 seconds to go from 0 to -0.4
