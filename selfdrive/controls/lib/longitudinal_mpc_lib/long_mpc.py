...
class LongitudinalMpc():
  ...
  def update_TF(self, carstate):
    if carstate.distanceLines == 1: # Traffic
      ...
      y_dist = [1.34, 1.34, 1.34,    1.35,  1.36,   1.37,    1.36,   1.3,   1.22,   1.14,   1.3]
      ...
      y_dist = [1.4,  1.4,  1.4,   1.405,  1.411, 1.423, 1.446, 1.5,  1.8]
      self.desired_TF = np.interp(carstate.vEgo, x_vel, y_dist)
      #self.desired_TF = 1.7
    else:
      x_vel = [0.0, 2.788,  5.56,  8.333,  11.11, 13.89, 19.44, 25.0, 41.67]
      y_dist = [1.38,   1.44,  1.44,   1.44,  1.45,   1.5,  1.8,   2.0,    2.3]
