...
class LongitudinalMpc():
  ...
  def update_TF(self, carstate):
    if carstate.distanceLines == 1: # Traffic
      ...
      y_dist = [1.23, 1.23, 1.25, 1.28, 1.37, 1.37,  1.29,  1.2,  1.3,  1.33,  1.2,  1.1,   1.1,   1.2,   1.25,  1.3,   1.31]
      ...
      x_vel = [0.0, 19.44, 41.67]  # velocities
      y_dist = [1.3781, 1.543, 1.8]
      self.desired_TF = np.interp(carstate.vEgo, x_vel, y_dist)
      #self.desired_TF = 1.7
    else:
      x_vel = [0.0, 19.44, 41.67]  # velocities
      y_dist = [1.3781, 1.698, 2.2]
