...
_DP_CRUISE_MIN_V =       [-1.00, -1.05, -1.10, -1.10, -1.00,  -0.90]
_DP_CRUISE_MIN_V_ECO =   [-1.00, -1.00, -1.00, -1.00, -0.90,  -0.80]
_DP_CRUISE_MIN_V_SPORT = [-1.10, -1.10, -1.15, -1.20, -1.00,  -1.00]
  ...
  def get_df(self, v_ego):
    ...
    if self.dp_following_profile_ctrl:
      if self.dp_following_profile == 0:
        ...
        x_vel =  [0.0,  11.1,  13.89,  19.7,  25.0,  41.67]
        y_dist = [0.85, 1.04,  1.12,   1.2,   1.23,   1.24]
        desired_tf = np.interp(v_ego, x_vel, y_dist)
    return desired_tf
