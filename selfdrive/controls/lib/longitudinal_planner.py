...
_DP_CRUISE_MIN_V =       [-0.025, -0.025, -0.15, -0.15, -0.40, -0.40, -1.00]
_DP_CRUISE_MIN_V_ECO =   [-0.020, -0.020, -0.10, -0.10, -0.35, -0.35, -1.00]
_DP_CRUISE_MIN_V_SPORT = [-0.030, -0.030, -0.20, -0.20, -0.45, -0.45, -1.00]
_DP_CRUISE_MIN_BP =      [0.,    3.11,   3.12,  10.,  10.01, 20.,   30.,   55.]

_DP_CRUISE_MAX_V =       [2.0, 2.0, 2.0, 2.0, 1.4, 1.1, 0.75, 0.55, 0.35, 0.14]
_DP_CRUISE_MAX_V_ECO =   [2.0, 2.0, 2.0, 2.0, 1.2, .76, 0.56, 0.36, 0.28, 0.09]
_DP_CRUISE_MAX_V_SPORT = [2.0, 2.0, 2.0, 2.0, 1.6, 1.5, 1.35, 0.90, 0.70, 0.5]
  ...
  def get_df(self, v_ego):
    ...
    if self.dp_following_profile_ctrl:
      if self.dp_following_profile == 0:
        x_vel =  [0.0,   3.0,   12.0,  19.7,  25.0,  41.67]
        ...
        x_vel =  [0.0,  3.0,   12.0,  19.7,   41.67]
        y_dist = [1.2,  1.2,  1.3,  1.4,    1.5 ]
        desired_tf = np.interp(v_ego, x_vel, y_dist)
      elif self.dp_following_profile == 2:
        x_vel =  [0.0,  3.0,   12.0,  19.7,   41.67]
        y_dist = [1.3,  1.3,  1.5,   1.6,   1.8]
        desired_tf = np.interp(v_ego, x_vel, y_dist)
    return desired_tf
