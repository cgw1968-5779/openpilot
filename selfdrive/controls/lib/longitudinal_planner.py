...
_DP_CRUISE_MIN_V =       [-0.55, -0.65, -0.70, -0.75, -0.65,  -0.45]
_DP_CRUISE_MIN_V_ECO =   [-0.53, -0.60, -0.65, -0.68, -0.58,  -0.40]
_DP_CRUISE_MIN_V_SPORT = [-0.9,  -1.0,  -1.0,  -0.90, -0.80,  -0.7]
  ...
  def get_df(self, v_ego):
    ...
    if self.dp_following_profile_ctrl:
      if self.dp_following_profile == 0:
        ...
        y_dist = [0.5, 1.20,   1.29,    1.34,   1.23,   1.34]
        desired_tf = np.interp(v_ego, x_vel, y_dist)
    return desired_tf
