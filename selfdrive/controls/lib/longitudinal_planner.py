...
_DP_CRUISE_MIN_V =       [-0.32,  -0.37,  -0.37, -0.43,  -0.60, -0.45]
_DP_CRUISE_MIN_V_ECO =   [-0.29,  -0.34,  -0.35, -0.40,  -0.55, -0.40]
_DP_CRUISE_MIN_V_SPORT = [-0.35,  -0.40,  -0.40, -0.50,  -0.75, -0.50]
_DP_CRUISE_MIN_BP =      [0.,     0.07,   3.1,   10.,   20.,   55.]

_DP_CRUISE_MAX_V =       [3.5, 3.5, 3.4, 2.6, 1.4, 0.99, 0.69, 0.45, 0.34, 0.13]
_DP_CRUISE_MAX_V_ECO =   [3.5, 3.5, 3.0, 2.2, 0.94, 0.72, 0.45, 0.36, 0.28, 0.09]
_DP_CRUISE_MAX_V_SPORT = [3.5, 3.5, 3.5, 3.0, 2.1, 1.61, 1.1,  0.63, 0.50, 0.33]
  ...
  def get_df(self, v_ego):
    ...
    if self.dp_following_profile_ctrl:
      if self.dp_following_profile == 0:
        x_vel =  [1.0,   19.7,   25.0,   41.67]
        y_dist = [0.96,  1.08,    1.23,   1.27]
        desired_tf = np.interp(v_ego, x_vel, y_dist)
      elif self.dp_following_profile == 1:
        x_vel =  [1.0,   5.556, 19.7,   41.67]
        y_dist = [1.0,   1.3,   1.6,    1.6 ]
        desired_tf = np.interp(v_ego, x_vel, y_dist)
      elif self.dp_following_profile == 2:
        x_vel =  [1.0,   5.556,  19.7,   41.67]
        y_dist = [1.0,   1.5,    2.0,    2.0]
        desired_tf = np.interp(v_ego, x_vel, y_dist)
    return desired_tf
