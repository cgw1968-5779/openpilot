...
_DP_CRUISE_MIN_V =       [-0.47,  -0.50,  -0.55, -0.60,  -0.65, -0.45]
_DP_CRUISE_MIN_V_ECO =   [-0.43,  -0.45,  -0.50, -0.55,  -0.60, -0.40]
_DP_CRUISE_MIN_V_SPORT = [-0.55,  -0.60,  -0.65, -0.70,  -0.75, -0.50]
  ...
  def get_df(self, v_ego):
    ...
    if self.dp_following_profile_ctrl:
      if self.dp_following_profile == 0:
        ...
        x_vel =  [1.0,   2.667,  5.555,  13.89,  16.67,  25.0,   41.67]
        y_dist = [0.85,   0.95,  0.98,   1.03,   1.1,    1.23,   1.27]
        desired_tf = np.interp(v_ego, x_vel, y_dist)
    return desired_tf
