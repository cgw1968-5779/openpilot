...
_DP_CRUISE_MIN_V =       [-0.86, -0.86, -0.86, -0.95, -0.95, -0.95]
_DP_CRUISE_MIN_V_ECO =   [-0.84, -0.84, -0.84, -0.86, -0.93, -0.90]
_DP_CRUISE_MIN_V_SPORT = [-0.88, -0.88, -0.88, -0.92, -0.98, -0.98]
_DP_CRUISE_MIN_BP =      [0.,    8.3,   12,    20.,   30.,   55.]

_DP_CRUISE_MAX_V =       [3.5, 3.5, 3.4, 2.5, 1.06, 0.90, 0.68, 0.45, 0.34, 0.13]
_DP_CRUISE_MAX_V_ECO =   [3.5, 3.5, 3.3, 2.0, .86, .72,  .53,   0.36, 0.28, 0.09]
_DP_CRUISE_MAX_V_SPORT = [3.5, 3.5, 3.5, 3.5, 2.6, 2.1, 1.3,  0.9, 0.7, 0.5]
  ...
  def get_df(self, v_ego):
    ...
    if self.dp_following_profile_ctrl:
      if self.dp_following_profile == 0:
        x_vel =  [0.0,   3.0,  3.01,  6.0,  6.01,  9.0,   9.01,  12.0,  12.01,  19.7,  25.0,  41.67]
        y_dist = [0.85,  0.85, 0.86,  0.86, 0.86,  0.86,  0.89,  0.89,  1.00,  1.00,   1.14,   1.23]
        #x_vel =  [0.0,  3.0,  3.01,  8.3,  8.31, 19.7, 25.0, 41.67]
        #y_dist = [0.91, 0.93, 1.11,  1.11, 1.16, 1.16, 1.20, 1.25]
        desired_tf = np.interp(v_ego, x_vel, y_dist)
      elif self.dp_following_profile == 1:
        x_vel =  [0.0,  5.55,  5.56,  9.00,  9.01,  13.9,  13.91,  19.7,   41.67]
        y_dist = [0.85, 0.85,  1.0,   1.0,   1.05,  1.05,  1.1,    1.2,    1.3 ]
        desired_tf = np.interp(v_ego, x_vel, y_dist)
      elif self.dp_following_profile == 2:
        x_vel =  [0.0,  3.0,  3.01,  13.9,  13.91,  19.7,   41.67]
        y_dist = [1.2,  1.2,  1.4,   1.4,   1.6,    2.0,    2.0]
        desired_tf = np.interp(v_ego, x_vel, y_dist)
    return desired_tf
