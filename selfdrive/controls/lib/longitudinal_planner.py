...
_DP_CRUISE_MIN_V =       [-0.90, -0.90, -0.90, -0.80, -0.70, -0.60]
_DP_CRUISE_MIN_V_ECO =   [-0.80, -0.80, -0.80, -0.70, -0.60, -0.50]
_DP_CRUISE_MIN_V_SPORT = [-1.00, -1.00, -1.00, -0.90, -0.80, -0.70]
  ...
  def get_df(self, v_ego):
    ...
    if self.dp_following_profile_ctrl:
      if self.dp_following_profile == 0:
        ...
        x_vel =  [0.0,  2.0,   5.5,   13.89,  19.7,   25.0,   41.67]
        y_dist = [0.80, 0.90,  1.1,   1.33,   1.42,   1.4,  1.4]
        desired_tf = np.interp(v_ego, x_vel, y_dist)
    return desired_tf
