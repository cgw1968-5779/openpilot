...
_DP_CRUISE_MIN_V =       [-1.00, -1.00, -1.00, -0.90, -0.80, -0.70]
_DP_CRUISE_MIN_V_ECO =   [-0.80, -0.90, -0.90, -0.80, -0.70, -0.60]
_DP_CRUISE_MIN_V_SPORT = [-1.10, -1.10, -1.10, -1.00, -0.90, -0.80]
  ...
  def get_df(self, v_ego):
    ...
    if self.dp_following_profile_ctrl:
      if self.dp_following_profile == 0:
        x_vel =  [0.0,  8.3,   19.7,   41.67]
        y_dist = [1.30, 1.45,  2.2,    2.2]
        desired_tf = np.interp(v_ego, x_vel, y_dist)
      elif self.dp_following_profile == 1:
        x_vel =  [0.0,  5.556, 19.7,   41.67]
        y_dist = [1.2, 1.35,   1.8,    1.8 ]
        desired_tf = np.interp(v_ego, x_vel, y_dist)
      elif self.dp_following_profile == 2:
        x_vel =  [0.0,  8.0,  8.33,  13.89,  19.7,   25.0,   41.67]
        y_dist = [0.85, 0.85,  1.33,  1.35,   1.42,   1.4,  1.4]
        desired_tf = np.interp(v_ego, x_vel, y_dist)
    return desired_tf
