...
_DP_CRUISE_MIN_V =       [-0.30, -0.36, -0.46, -0.56, -0.66, -0.76, -1.00]
_DP_CRUISE_MIN_V_ECO =   [-0.28, -0.34, -0.44, -0.54, -0.64, -0.74, -1.00]
_DP_CRUISE_MIN_V_SPORT = [-0.32, -0.38, -0.48, -0.58, -0.68, -0.78, -1.00]
  ...
  def get_df(self, v_ego):
    ...
    if self.dp_following_profile_ctrl:
      if self.dp_following_profile == 0:
        x_vel =  [0.0,   0.03,   12.0,  19.7,  25.0,  41.67]
        ...
        x_vel =  [0.0,  0.03,   12.0,  19.7,   41.67]
        y_dist = [1.3,  1.3,   1.45,  1.5,    1.5 ]
        desired_tf = np.interp(v_ego, x_vel, y_dist)
      elif self.dp_following_profile == 2:
        x_vel =  [0.0,  0.03,  19.7,   41.67]
        y_dist = [1.5,  1.5,  1.8,   1.8]
        desired_tf = np.interp(v_ego, x_vel, y_dist)
    return desired_tf
