...
_DP_CRUISE_MIN_V =       [-0.85, -0.85, -0.90, -1.00, -0.90,  -0.80]
_DP_CRUISE_MIN_V_ECO =   [-0.80, -0.80, -0.85, -0.90, -0.80,  -0.70]
_DP_CRUISE_MIN_V_SPORT = [-0.90, -0.90, -1.00, -1.10, -1.00,  -0.90]
  ...
  def get_df(self, v_ego):
    ...
    if self.dp_following_profile_ctrl:
      if self.dp_following_profile == 0:
        ...
        y_dist = [0.80,  0.80, 1.12,  1.12,  1.18,   1.2,   1.21,   1.2]
        desired_tf = np.interp(v_ego, x_vel, y_dist)
    return desired_tf
