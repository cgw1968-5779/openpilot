...
_DP_CRUISE_MIN_V =       [-0.22, -0.22, -0.26, -0.26, -0.46, -0.46, -0.66, -1.00, -1.00]
_DP_CRUISE_MIN_V_ECO =   [-0.20, -0.20, -0.24, -0.24, -0.44, -0.44, -0.64, -1.00, -1.00]
_DP_CRUISE_MIN_V_SPORT = [-0.28, -0.28, -0.28, -0.28, -0.48, -0.48, -0.68, -1.02, -1.02]
_DP_CRUISE_MIN_BP =      [0.,     0.05, 0.06,  3.11,  3.12,  10.,   10.01, 20.,   30.]

_DP_CRUISE_MAX_V =       [2.0, 2.0, 2.0, 1.4, 0.95, 0.70, 0.54, 0.34, 0.13]
_DP_CRUISE_MAX_V_ECO =   [2.0, 2.0, 1.9, 1.17, .755, 0.555, 0.36, 0.28, 0.09]
_DP_CRUISE_MAX_V_SPORT = [2.0, 2.0, 2.0, 1.6, 1.5, 1.35, 0.90, 0.70, 0.5]
  ...
  def get_df(self, v_ego):
    ...
    if self.dp_following_profile_ctrl:
      if self.dp_following_profile == 0:
        x_vel =  [0.0,   0.1,   12.0,  19.7,  25.0,  41.67]
        y_dist = [1.1,   1.1,  1.1,  1.1,  1.15,  1.22]
        ...
        x_vel =  [0.0,  0.1,   12.0,  19.7,   41.67]
        y_dist = [1.0,  1.0,   1.45,  1.5,    1.5 ]
        desired_tf = np.interp(v_ego, x_vel, y_dist)
      elif self.dp_following_profile == 2:
        x_vel =  [0.0,  0.1,  19.7,   41.67]
        y_dist = [1.0,  1.0,  1.8,   1.8]
        desired_tf = np.interp(v_ego, x_vel, y_dist)
    return desired_tf
