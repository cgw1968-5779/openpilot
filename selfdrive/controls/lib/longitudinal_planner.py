...
_DP_CRUISE_MIN_V =       [-0.45,  -0.45,  -0.50,  -0.45, -0.40]
_DP_CRUISE_MIN_V_ECO =   [-0.40,  -0.40,  -0.45,  -0.40, -0.35]
_DP_CRUISE_MIN_V_SPORT = [-0.50,  -0.50,  -0.55,  -0.50, -0.50]
_DP_CRUISE_MIN_BP =      [0.,     3.1,    10.,    20.,   36.]

_DP_CRUISE_MAX_V =       [3.5, 3.4, 2.1, 1.6, 1.10, 0.90, 0.68, 0.45, 0.34, 0.13]
  ...
  def get_df(self, v_ego):
    ...
    if self.dp_following_profile_ctrl:
      if self.dp_following_profile == 0:
        ...
        y_dist = [1.3,   1.5,    2.0,    2.0]
        desired_tf = np.interp(v_ego, x_vel, y_dist)
      elif self.dp_following_profile == 1:
        x_vel =  [1.0,   5.556, 19.7,   41.67]
        y_dist = [1.3,   1.3,   1.5,    1.5 ]
        desired_tf = np.interp(v_ego, x_vel, y_dist)
      elif self.dp_following_profile == 2:
        x_vel =  [1.0,   5.556,  13.89,  16.67,  25.0,   41.67]
        y_dist = [1.191, 1.000,  1.000,  1.133,  1.22,   1.25]
        desired_tf = np.interp(v_ego, x_vel, y_dist)
    return desired_tf
