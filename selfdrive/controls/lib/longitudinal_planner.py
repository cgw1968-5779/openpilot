...
_DP_CRUISE_MIN_V =       [-1.00, -1.00, -1.00, -1.00, -0.90,  -0.80]
_DP_CRUISE_MIN_V_ECO =   [-0.90, -0.90, -0.90, -0.90, -0.80,  -0.70]
_DP_CRUISE_MIN_V_SPORT = [-1.10, -1.10, -1.10, -1.10, -1.00,  -0.90]
  ...
  def get_df(self, v_ego):
    ...
    if self.dp_following_profile_ctrl:
      if self.dp_following_profile == 0:
        x_vel =  [1.0,  8.3,   19.7,   41.67]
        y_dist = [1.00, 1.35,  2.0,    2.0]
        desired_tf = np.interp(v_ego, x_vel, y_dist)
      elif self.dp_following_profile == 1:
        x_vel =  [1.0,  5.556, 19.7,   41.67]
        y_dist = [1.00, 1.34,   1.5,    1.6 ]
        desired_tf = np.interp(v_ego, x_vel, y_dist)
      elif self.dp_following_profile == 2:
        x_vel =  [1.0,  2.0,   11.1,   13.89,  19.7,   25.0,   41.67]
        y_dist = [0.90, 0.95,  0.95,   1.05,   1.19,   1.224,  1.23]
        desired_tf = np.interp(v_ego, x_vel, y_dist)
    return desired_tf
